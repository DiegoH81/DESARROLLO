...
class ProductModel():
    ...
    def create_product(self, data_base, product_obj):
        ...
        sql_insert_product = """INSERT INTO test_flask.productos (id_usuario, nombre_producto, decripcion, precio)
                                 VALUES ('{}', '{}', '{}', '{}')""".format(product_obj.user_id, product_obj.nombre, product_obj.descripcion, product_obj.precio)
    ...
    def get_by_id(self, data_base, id):
        cursor = data_base.connection.cursor()
        sql_get_id = "SELECT id, id_usuario, nombre_producto, decripcion, precio FROM test_flask.productos WHERE id = {}".format(id)
        cursor.execute(sql_get_id)
        ROW = cursor.fetchone()
        
        if ROW != None: #Si existe el producto
            return Product(ROW[0], ROW[1], ROW[2], ROW[3], ROW[4]) #Reotrno el producto con los datos cargados
        else:
            return None
